...
const CustomStylesMenu = ({navigation, route}) => {
    ...
    const newStyleTextBoxForNameRef = useRef(); //Allows for auto focus of text when creating a new style
    ...
    const UserTextInput = ({label, icon, isPassword, newStyleCreation, ...props}) => {
        ...
        return(
            <SearchBarArea>
                ...
                <StyledTextInput style={{backgroundColor: colors.primary, color: colors.tertiary, borderColor: colors.borderColor}} searchPage={true} ref={newStyleTextBoxForNameRef} {...props}/>
            </SearchBarArea>
        )
    }
        ...
        async function getSimpleStylingData() {
            ...
            if (amountOfStylesToGetUpdated == 0 || forceRefresh == true) {
                ...
                for (let i = 0; i < data_parsed.length; i++) {
                    try {
                        if (data_parsed[i].stylingVersion < StylingVersion) {
    ...
    const handlePress = (indexNum, forceStyle) => {
        ...
        if (forceStyle == true || StylingVersion == simpleStylingData[indexNumToUse].stylingVersion) {
            setRefreshAppStyling(true);
        ...
        } else if (StylingVersion < simpleStylingData[indexNumToUse].stylingVersion) {
        ...
        } else if (simpleStylingData[indexNumToUse].stylingVersion == undefined || StylingVersion > simpleStylingData[indexNumToUse].stylingVersion) {
            let stylingVersionUsed = simpleStylingData[indexNumToUse].stylingVersion;
            setVersionMismatchScreenVersion(stylingVersionUsed);
    ...
    const showOrHideNamingNewStyle = () => {
        setNamingNewStyle(NamingNewStyle == true ? false : true)
    }

    useEffect(() => { // Once the naming new style screen has shown up, focus the text box for the new style name
        if (NamingNewStyle == false) {
            newStyleTextBoxForNameRef.current.focus();
        }
    }, [NamingNewStyle])
    return (
        <>
            <ChatScreen_Title style={{backgroundColor: colors.primary, borderWidth: 0, marginBottom: -40}}>
                ...
                {confirmUpdateScreenHidden == true && versionMismatchScreenHidden == true && outdatedStyleWarningWhenEditingHidden == true &&
                    <TouchableOpacity onPress={() => {versionReleaseNotesHidden == true ? showOrHideNamingNewStyle() : setVersionReleaseNotesHidden(true)}} style={{position: 'absolute', right: versionReleaseNotesHidden == false ? 15 : NamingNewStyle == true ? 10 : 15, top: StatusBarHeight + 2}}>
                        {NamingNewStyle == true ?
            ...
            <ProfileOptionsView style={{backgroundColor: colors.primary}} viewHidden={versionMismatchScreenHidden}>
                <View style={{flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
                    <Text style={{color: colors.errorColor, fontSize: 18, fontWeight: 'bold', textAlign: 'center', marginHorizontal: 10}}>This style is version {versionMismatchScreenVersion || 'ERROR'} and this version of SocialSquare only supports custom styles with version {StylingVersion}.</Text>
            ...
            <ProfileOptionsView style={{backgroundColor: colors.primary, height: '80%'}} viewHidden={versionReleaseNotesHidden}>
                <View style={{width: '90%', height: '100%'}}>
                    <ScrollView>
                        <Text style={{color: colors.tertiary, fontSize: 24, fontWeight: 'bold', textAlign: 'center'}}>{'Release Notes for simple styling version ' + versionReleaseNotesVersion}</Text>
                        {versionReleaseNotesVersion == 1 ?
                            <>
                                ...
                                <Text style={{color: colors.tertiary, fontSize: 18, textAlign: 'center', marginVertical: 7}}>{`\u2022 Added full color customization for SocialSquare Desktop (styles also compatible with SocialSquare mobile)`}</Text>
                        ...
                        <>
                            {amountOfStylesToGetUpdated != null && confirmUpdateScreenHidden == true && versionMismatchScreenHidden == true && versionReleaseNotesHidden == true && amountOfStylesToGetUpdated != 0 && outdatedStyleWarningWhenEditingHidden == true && NamingNewStyle == true &&
                                <TouchableOpacity onPress={() => {confirmUpdateScreenHidden == true ? setConfirmUpdateScreenHidden(false) : setConfirmUpdateScreenHidden(true)}} style={{padding: 5, backgroundColor: colors.primary, justifyContent: 'center', alignSelf: 'center', alignItems: 'center', borderColor: colors.borderColor, borderWidth: 3, borderRadius: 10, marginBottom: 10}}>
                                    <Text style={{fontSize: 20, fontWeight: 'bold', color: colors.tertiary, textAlign: 'center'}}>{displayName || 'Cannot find name'} has {amountOfStylesToGetUpdated} {amountOfStylesToGetUpdated == 1 ? 'style' : 'stylings'} that {amountOfStylesToGetUpdated == 1 ? 'needs' : 'need'} to be updated to V{StylingVersion}</Text>
...
export default CustomStylesMenu;