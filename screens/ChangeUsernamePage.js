...
import {View, ActivityIndicator, ImageBackground, StyleSheet, ScrollView, TouchableOpacity, Image} from 'react-native';
...
const ChangeUsernamePage = ({navigation}) => {
    ...
    const handleChangeDesiredUsername = (credentials, setSubmitting) => {
        ...
        axios.post(url, credentials).then((response) => {
            ...
            if (status !== 'SUCCESS') {
                ...
                persistLogin(credentials.desiredUsername);
            }
            setSubmitting(false);
    ...
    const persistLogin = (desiredUsername) => {
        let tempCredentials = storedCredentials;
        tempCredentials.name = desiredUsername;

        AsyncStorage.setItem('socialSquareCredentials', JSON.stringify(tempCredentials))
        .then(() => {
            handleMessage('Changed username successfully.');
            setStoredCredentials(tempCredentials);
        })
        .catch((error) => {
            console.log(error);
